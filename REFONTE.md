...
> Dernière mise à jour : **2026-04-20** — sub-palier 5.3 complet (page Overview avec data réelle). Sidebar dynamique, KPIs live, graphes uPlot avec range selector, grille de modules. Page Overview entièrement opérationnelle.
...
- **2026-04-20** — Feature 5.2.2 ✅ : layout HTML + template engine. Package `internal/dashboard/web/` étoffé : `renderer.go` (wrap `html/template` avec cache `sync.RWMutex` + dev-mode `DASHBOARD_DEV_MODE` hot reload, prod-mode eager parse au startup), `funcs.go` (FuncMap avec `fmtDuration` : jours/heures/minutes/secondes compacts, `—` pour négatif). `templates/layout.html` wrappé dans `{{define "layout"}}...{{end}}` (pour que `ExecuteTemplate(w,"layout",…)` trouve le bon template — piège du naming par basename) avec `{{block "content" .}}` comme extension point. `templates/home.html` définit juste `{{define "content"}}...{{end}}`. Sidebar hardcodée (logo, machine switcher `vps`, nav Overview + 3 modules, Config en bas), 3 KPI cards placeholders dont une utilisant `fmtDuration` = `2d 14h 22m`. Tailwind via jsdelivr CDN, HTMX local `/static/htmx.min.js`. `NewRouter` retourne désormais `error` (parse failure = fail fast). `main.go` ajoute `getEnvBool` helper. 9 nouveaux tests web (6 fmtDuration, 4 renderer, +2 home HTML/sidebar) → package web à **15 PASS**. E2E : HTML complet servi, DOCTYPE présent, `2d 14h 22m` rendu, tous les éléments sidebar détectés. Hotfix : URL jsdelivr `tailwindcss@3/dist/tailwind.min.css` retourne 404 (Tailwind v3 ne livre plus de CSS prébuilt) — switch vers Play CDN officiel `cdn.tailwindcss.com` (transitoire avant 5.2.3).
- **2026-04-20** — Feature 5.2.3 ✅ : tokens CSS + polices Geist + autonomie totale. Nouveau `static/css/app.css` (~330 lignes) implémente `design/design-tokens.md` : 4 backgrounds, 3 borders, 4 text levels, accent teal en `oklch()` avec variantes bg/border, sémantiques ok/warn/crit, typography (`--fs-xs..3xl`, `--font-sans/mono`), spacing (`--s-1..12`), radius (`--r-sm..xl`), layout (`--sidebar-w`, `--topbar-h`). `@font-face` charge Geist + Geist Mono Variable WOFF2 depuis `/static/fonts/` avec `font-display: swap`. `layout.html` et `home.html` migrés de Tailwind/inline vers classes propres (`.app-shell`, `.sidebar`, `.brand`, `.nav-item`, `.card`, `.kpi-card`, `.status-dot.ok`). **Tailwind retiré** — dashboard 100% self-contained, zéro CDN externe, airplane test passe (`href`/`src` uniquement `/static/...`). 3 nouveaux tests (CSS servi avec token + Geist, fonts >10KB, home sans Tailwind/jsdelivr + classes attendues). Package web : **18 PASS**.
- **2026-04-20** — 🎉 **Sub-palier 5.2 VALIDÉ**. Web scaffolding complet : refactor api+web sous un seul `*http.Server`, package `web/` avec `renderer.go` + `funcs.go`, layout HTML avec sidebar, design tokens du `design-tokens.md` appliqués, polices Geist embeddées, dashboard auto-suffisant. ~700 lignes Go + 350 lignes CSS + 100 lignes HTML. 18 tests web verts. Prêt pour 5.3 (binding des vraies données API → templates).
- **2026-04-20** — Fix UI : icônes Unicode `⌂◫▣◈⚙` remplacés par 5 SVG Lucide inline (`partials/icon.html` + `fs.Glob` dans `renderer.load`). Brand padding bumpé à `var(--s-5)` + `min-height: 64px` pour l'alignement vertical avec la section machine en-dessous.
- **2026-04-20** — Feature 5.3.1 ✅ : sidebar dynamique + navigation par machine. (1) `GET /` → 303 redirect vers `/agents/{first-alphabetical}` ou rendu no-agents si DB vide. (2) `GET /agents/{id}` → page Overview stub ou 404. (3) `sidebar.go` centralise `buildSidebar` + `firstMachineID` (réutilisable par tous les handlers). (4) Partials `machine-switcher` + `machine-entry` : dropdown `<details>` seulement si `len(machines) > 1`, sinon `.machine-switcher-static` (pas d'affordance vide). (5) Modules nav items conservés visuellement mais `.disabled` + `aria-disabled="true"` (Coming in 5.4). (6) `renderer.RenderStandalone` exécute `"standalone"` au lieu de `"layout"` — utilisé par `no_agents.html` qui n'a pas de shell. (7) `funcMap` expose `currentMachine` ; `$.CurrentMachineID` utilisé dans `{{range .Machines}}` pour accéder au contexte parent. (8) `web.Config` gagne `OfflineThreshold` + `Version`, threadés depuis `main.go` (réutilise la var `version` ldflags-injectable). (9) Suppression de `home.html`. CSS : `.nav-item.disabled`, `.machine-switcher-dropdown/static`, `.machine-list`, `.empty-state`, `.code-block`. Tests : 5 nouveaux `overview_test.go` (404, single→pas de dropdown, multi→dropdown + cross-link, disabled items, title), `RendersOverview` + `RendersStandalone` dans renderer, redirect+location assert dans home. Total package web : **21 PASS**. E2E : single agent → static switcher, fake `pi` injecté → `<details` rendu + `href="/agents/pi"`, DB vidée → "No agents yet" sans app-shell.
- **2026-04-20** — Feature 5.3.2 ✅ : page Overview avec topbar + 6 KPI cards. Nouveau `kpis.go` (builders typés Uptime/Load/Memory/Disk/Containers/Last push avec stricte séparation calc-Go/render-HTML). Handler enrichi : charge `GetLatestMetrics` + réutilise `sidebar.LastPushAt` (pas de 2e query DB). 3 helpers template ajoutés : `fmtBytes` (décimal SI, 0/1/1/2 décimales par tier), `fmtPercent` (`"%.0f%%"`), `fmtRelativeDuration` (`Xs ago`/`Xm ago`/`Xh Xm ago`). `variantFromPercent` colorise en `.kpi-warn` à ≥85% / `.kpi-crit` à ≥95%. Topbar = breadcrumb seul (auto-refresh + horloge reportés au 5.6 — pas de placeholder figé pour ne pas mentir visuellement). Template overview avec `{{range .KPIs.Cards}}{{template "kpi-card" .}}{{end}}` DRY. CSS responsive : grille 6 cols → 3 cols `<1200px` → 2 cols `<700px`. **`sidebarMachine` enrichi** avec `LastPushAt` + `FirstSeenAt` (réutilisé par d'autres pages futures sans re-query). 11 nouveaux tests kpis (builders complete/all-missing/partial, warn/crit/zero-total, fmtBytes 10 cas, fmtPercent 4 cas, fmtRelativeDuration 6 cas, variantFromPercent 6 cas) + 2 tests overview (KPIsRendered, MissingMetricsShowDashes). Total web : **33 PASS**. E2E live (vps réel) : Uptime `1d 1h 17m`, Load `2.81` (5m avg 1.83), Memory `32%` (9.4/29.2 GB), Disk / `59%` (594.4 GB / 1.01 TB), Containers `—`, Last push `26s ago`.
- **2026-04-20** — Feature 5.3.3 ✅ : graphes historiques uPlot + range selector. uPlot v1.6.31 embed local (`/static/lib/`, airplane test maintenu). Nouveau `static/js/overview.js` (~210 L IIFE) : `fetchPoints` + `Promise.all` parallèle, `pointsToUPlotData` aligne défensivement les timestamps multi-séries, `destroy()+recreate` au range change pour éviter les leaks, init gardé (return early si containers absents → safe sur home/no_agents). 2 graphes : Load average (3 lignes 1m/5m/15m) + Memory available (1 ligne). Range selector `[1h] [24h] [7d]` JS pur (pas de HTMX), `7d → "168h"` workaround pour le parser API qui ne supporte pas `d`. Y-axis : `fmtBytesShort` (KB/MB/GB/TB) pour Memory, `toFixed(2)` pour Load — évite `2.9e+10` cryptique. Hotfix : `fmtXAxis` ajouté (HH:MM pour ≤24h, DD/MM HH:MM pour 7d, en local time via `Date.getHours/getDate`) car les ticks UTC par défaut d'uPlot étaient illisibles vs l'horloge locale. CSS `charts-section`, `range-selector` (active state), `chart-card`, `chart-status` (avec `.chart-status-error` en `--crit`), overrides uPlot alignés sur tokens. `layout.html` charge uPlot CSS+JS + overview.js avec `defer`. Pas de tests Go nouveaux (consomme l'API existante). E2E : assets servis (50K JS, 1.8K CSS, 6.6K overview.js), markup DOM présent, API renvoie 104 points sur `range=24h`.
- **2026-04-20** — Feature 5.3.4 ✅ : grille des modules actifs. Storage : `ListModulesForAgent(ctx, machineID)` via `SELECT module, COUNT(DISTINCT name), MAX(timestamp_ns) GROUP BY module ORDER BY module`. Type public `ModuleInfo {Module, MetricCount, LastUpdateAt}`. Nouveau `web/modules.go` : `buildModuleCards` fusionne modules détectés (active, ordre alpha) + liste hardcodée `comingSoonModules` (Docker→5.4, Certs→5.5) avec dédup défensif (un module devenu active disparaît auto de la liste coming au cas où on oublie de le retirer manuellement). Pattern `displayName` (capitalize première lettre). Partial `module-card` avec switch `if/eq .Icon` car `html/template` ne supporte pas les noms de template dynamiques. Nouvelle icône `icon-activity` (Lucide). CSS : `.modules-section`, `.card-row.cols-4` (responsive 4→2→1), `.module-card` avec `.module-coming` opacité réduite + cursor not-allowed, status-dot pushed via `margin-left:auto`. Tous les liens `href="#"` + `aria-disabled="true"` jusqu'au 5.4+. **Décision UX** : refusé d'afficher des placeholders "futurs non planifiés" (Backups/ZFS) — éviter le wishful thinking dans l'UI. 4 nouveaux tests storage + 5 nouveaux tests web. Storage : **40 PASS**, web : **38 PASS** (parent-level). E2E live : 4 cards (Heartbeat + System actifs avec icônes activity/server, Docker + Certs coming avec container/shield-check).
- **2026-04-20** — 🎉 **Sub-palier 5.3 VALIDÉ**. Page Overview entièrement opérationnelle : sidebar dynamique avec dropdown machine, breadcrumb, titre mono, 6 KPI cards live (Uptime/Load/Memory/Disk/Containers placeholder/Last push) avec variants warn/crit, 2 graphes historiques uPlot (Load 3 lignes + Memory available) avec range selector 1h/24h/7d et X-axis local time, grille de 4 modules cards (active + coming-soon). 4 features (5.3.1 → 5.3.4), ~1200 lignes Go + 700 lignes CSS + 250 lignes HTML/JS. Total tests dashboard : 40 storage + 38 web = 78 PASS. Pipeline data complet visible bout-en-bout : agent push → SQLite → API JSON → templates serveur → UI rendue.
- **2026-04-20** — 🏷️ **Release `v0.5.0-mvp` taguée**. 15 features livrées sur la branche `palier-5-dashboard`, mergée en fast-forward dans `main` : storage SQLite complet (5.0.1 → 5.0.4), API JSON read-only avec 4 endpoints (5.1.1 → 5.1.4), front HTMX + templates Go self-contained (5.2.1 → 5.2.3), page Overview branchée bout-en-bout (5.3.1 → 5.3.4). Pipeline : agent push 30s → SQLite WAL → API JSON → UI rendue. Dashboard airplane-testable (zéro CDN). Reste à faire dans le palier 5 : module Docker (5.4), Config (5.5), refresh live HTMX (5.6), états dégradés (5.7).
